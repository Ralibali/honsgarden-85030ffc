  ...
  try {
    ...
    return commits
      .map((row) => {
        ...
        if (!row.sha || !occurredAt || !label) return null;
        return {
          id: row.sha,
          label,
      ...
      .filter((row): row is GrowthChangeMarker => row !== null)
      .filter((row) => /^(feat|fix|perf|refactor|merge|release|chore\(deps\)|chore\(release\))/i.test(row.label))
      .slice(0, 20);
  } catch {
    return [];
  }
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <CardTitle className="font-serif text-lg flex items-center gap-2">
                ...
                Veckans Growth Brief
              </CardTitle>
              <p className="text-xs text-muted-foreground mt-1">
                Deterministiska fakta först. AI får tolka, aldrig hitta på mätdata.
              </p>
            </div>
            ...
            <div key={step.key} className="grid grid-cols-[120px_1fr_62px] gap-3 items-center">
              <div>
                ...
                {step.dropFromPreviousPct != null && (
                  <p className="text-[9px] text-muted-foreground">−{step.dropFromPreviousPct}% från föregående</p>
                )}
              </div>
          ...
          <table className="w-full min-w-[720px] text-xs">
            <thead>
              <tr className="border-b border-border text-muted-foreground text-left">
                <th className="py-2 pr-3">Signup-vecka</th>
                <th className="py-2 px-2 text-right">N</th>
                <th className="py-2 px-2 text-right">Höna ≤7d</th>
                <th className="py-2 px-2 text-right">Ägg ≤7d</th>
                <th className="py-2 px-2 text-right">D1</th>
                <th className="py-2 px-2 text-right">D7</th>
                <th className="py-2 pl-2 text-right">D30</th>
              </tr>
            </thead>
            <tbody>
              {intelligence.cohorts.slice().reverse().map((cohort) => (
                <tr key={cohort.week} className="border-b border-border/40 last:border-0">
                  <td className="py-2 pr-3 font-medium">{cohort.week}</td>
                  <td className="py-2 px-2 text-right">{cohort.signups}</td>
                  <td className="py-2 px-2 text-right">{cohort.firstHen7Pct ?? '—'}{cohort.firstHen7Pct != null ? '%' : ''}</td>
                  <td className="py-2 px-2 text-right">{cohort.firstEgg7Pct ?? '—'}{cohort.firstEgg7Pct != null ? '%' : ''}</td>
                  <td className="py-2 px-2 text-right">{retentionLabel(cohort.d1)}</td>
                  <td className="py-2 px-2 text-right">{retentionLabel(cohort.d7)}</td>
                  <td className="py-2 pl-2 text-right">{retentionLabel(cohort.d30)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-2">
          <CardTitle className="font-serif text-sm flex items-center gap-2">
            {intelligence.anomalies.some((a) => a.direction === 'down')
              ? <TrendingDown className="h-4 w-4 text-red-500" />
              : <TrendingUp className="h-4 w-4 text-green-500" />}
            Automatisk anomalidetektion
          </CardTitle>
          <p className="text-[10px] text-muted-foreground">
            Jämför två avslutade kalenderveckor. Minst 25% och ≥2 användares skillnad krävs.
          </p>
        </CardHeader>
        <CardContent>
          {intelligence.anomalies.length ? (
              ...
              {intelligence.anomalies.map((anomaly) => (
                <div key={anomaly.metric} className="flex items-center gap-3 rounded-xl border border-border/60 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold">{anomaly.label}</p>
                    <p className="text-[10px] text-muted-foreground">{anomaly.before} → {anomaly.after}</p>
                  </div>
                  <Badge variant="outline" className={anomaly.direction === 'up' ? 'text-green-700' : 'text-red-700'}>
            ...
            <p className="text-xs text-muted-foreground py-2">Ingen materiell vecka-mot-vecka-anomali. Tystnad är ett giltigt resultat.</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-2">
          ...
          <p className="text-[10px] text-muted-foreground">
            Kodändringar på main blir markörer. 7 dagar före/efter jämförs – korrelation ≠ kausalitet.
          </p>
        </CardHeader>
        <CardContent>
          {intelligence.changeCorrelations.length ? (
            <div className="space-y-2">
              {intelligence.changeCorrelations.slice(0, 8).map((change) => (
                <div key={change.id} className="rounded-xl border border-border/60 p-3">
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate">{change.label}</p>
                      <p className="text-[10px] text-muted-foreground mt-0.5">{change.occurredAt.slice(0, 10)} · {change.note}</p>
                    </div>
                    <Badge variant="outline" className={`text-[9px] ${correlationTone(change.status)}`}>
                    ...
                    {change.url && (
                      <a href={change.url} target="_blank" rel="noreferrer" aria-label="Öppna commit">
                        <ArrowUpRight className="h-4 w-4 text-muted-foreground" />
                      </a>
                    )}
                  </div>
                  {change.activeUsersBefore != null && change.activeUsersAfter != null && (
                    <p className="text-[10px] text-muted-foreground mt-2">
                      Aktiva användare: {change.activeUsersBefore} före → {change.activeUsersAfter} efter
                      {change.confounded ? ' · flera ändringar nära i tid' : ''}
                    </p>
                  )}
                </div>
              ))}
            ...
            <p className="text-xs text-muted-foreground py-2">GitHub-markörer kunde inte hämtas. Produktanalysen fungerar ändå.</p>
          )}
        </CardContent>
      </Card>

      <Card className="border-border/50">
        <CardHeader className="pb-2"><CardTitle className="font-serif text-sm">Datakvalitet & tolkning</CardTitle></CardHeader>
        <CardContent className="space-y-1.5">
          {intelligence.warnings.map((warning) => (
            <p key={warning} className="text-[11px] text-muted-foreground flex gap-2"><span>•</span>{warning}</p>
          ))}
        </CardContent>
      </Card>
    </div>
...
export default function GrowthIntelligence() {
  ...
  const growthData = useQuery({
    queryKey: ['growth-intelligence-data'],
    queryFn: fetchGrowthData,
    enabled: adminCheck.data?.is_admin === true,
    staleTime: 5 * 60 * 1000,
  });
  const changeMarkers = useQuery({
    queryKey: ['growth-change-markers'],
    queryFn: fetchGithubChangeMarkers,
    enabled: adminCheck.data?.is_admin === true,
  ...
  const intelligence = useMemo(() => {
    ...
    return buildGrowthIntelligence({
      ...growthData.data,
      changes: changeMarkers.data ?? [],
      observationStart: growthData.data.since,
      now: new Date(),
    });
  ...
  if (!adminCheck.data?.is_admin) {
    return (
      <div className="max-w-xl mx-auto py-16 text-center space-y-3">
        <Shield className="h-10 w-10 text-destructive/60 mx-auto" />
        <h1 className="font-serif text-xl">Åtkomst nekad</h1>
        <p className="text-sm text-muted-foreground">Growth Intelligence är endast tillgängligt för administratörer.</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex-1">
          <Link to="/app/admin" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground mb-2">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Retention cohorts, beteendemässig churn, funnel-läckor, anomalier och change→result i ett lager.
          </p>
        </div>
        <Button
          variant="outline"
          ...
          onClick={() => {
            queryClient.invalidateQueries({ queryKey: ['growth-intelligence-data'] });
            queryClient.invalidateQueries({ queryKey: ['growth-change-markers'] });
          }}
        >
      ...
      {growthData.isLoading ? (
        <div className="flex items-center justify-center py-20 text-sm text-muted-foreground">
          <Loader2 className="h-5 w-5 animate-spin mr-2" /> Bygger growth-modellen…
        </div>
      ) : growthData.isError ? (
        <Card className="border-destructive/30"><CardContent className="p-4 text-sm text-destructive">Kunde inte läsa growth-data: {String(growthData.error)}</CardContent></Card>
      ) : intelligence ? (
        <GrowthDashboard intelligence={intelligence} />
      ) : null}
