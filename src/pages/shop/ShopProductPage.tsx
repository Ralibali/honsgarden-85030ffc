...
export default function ShopProductPage() {
  ...
  const cartTotalOre = useMemo(() => {
    return cart.reduce((sum, item) => {
      ...
      return sum + (p ? item.quantity * p.price_ore : 0);
    }, 0);
  }, [cart, allProducts]);
  ...
  if (!product) {
    return (
      <div className="min-h-dvh bg-background font-outfit flex items-center justify-center p-8 text-center">
        <div className="max-w-lg">
          <h1 className="font-playfair text-3xl mb-4">Produkten hittades inte</h1>
  ...
  return (
    <div className="min-h-dvh bg-background font-outfit">
      <div className="max-w-6xl mx-auto px-4 py-4">
        <Button variant="ghost" size="sm" onClick={() => nav(-1)} className="mb-2 rounded-sm text-xs uppercase tracking-widest font-semibold text-muted-foreground hover:text-foreground">
      ...
      <div className="max-w-6xl mx-auto px-4 pb-12 grid md:grid-cols-2 gap-8">
        {/* Gallery */}
        <div>
          <div className="aspect-square rounded-sm bg-white border overflow-hidden flex items-center justify-center">
            {images.length > 0 ? (
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto">
              {images.map((src, i) => (
                <button
                  ...
                  className={`shrink-0 w-16 h-16 rounded-sm overflow-hidden border-2 ${i === imgIdx ? 'border-primary' : 'border-transparent'}`}
                >
        ...
        <div>
          ...
          <h1 className="font-playfair text-3xl md:text-4xl">{product.name}</h1>
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <Button size="lg" onClick={handleAdd} disabled={!inStock} className="flex-1 rounded-sm bg-primary text-primary-foreground h-auto py-4 text-xs uppercase tracking-widest font-semibold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:transform-none">
          ...
          {product.features.length > 0 && (
            <div className="mt-6">
              <h3 className="font-playfair text-lg mb-2">Höjdpunkter</h3>
          ...
          {product.specifications && Object.keys(product.specifications).length > 0 && (
            <div className="mt-6">
              <h3 className="font-playfair text-lg mb-2">Specifikationer</h3>
      ...
      {related.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 pb-16">
          <h2 className="font-playfair text-2xl mb-10">Kanske också intressant</h2>
