 ...
 *
 * VIKTIGT: Håll denna fil i sync med `src/lib/blogMarkdown.ts` (samma logik).
...
const stripInlineMarks = (text = '') => String(text).replace(/[#*_`]/g, '').trim();
 ...
 */
export function stripDuplicateTitleHeading(html = '', postTitle = '') {
...
export const BREED_FIGURES = [
  ...
  { match: 'blommehöna', image: '/blog-images/breeds/blommehona.jpg', label: 'Skånsk blommehöna', alt: 'Blomming skånsk blommehöna i trädgård' },
 ...
 */
export function injectBreedFigures(html = '') {
  const used = new Set();
...
export function heroForPost(post) {
