...
export default function OfflineBanner() {
  const { isOnline, pendingCount, legacyCount, syncing, syncError, retrySync, claimLegacy } = useOnlineStatus();
    ...
    return (
      <div role="status" className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-3 text-sm flex flex-wrap items-center justify-center gap-3">
        <span>{syncError || (pendingCount > 0 ? `${pendingCount} loggningar väntar på synk.` : '')}
          {legacyCount > 0 && ` ${legacyCount} äldre loggningar sparades innan du loggade in. Är de dina kan du spara dem på ditt konto.`}
        </span>
        ...
        {legacyCount > 0 && (
          <button className="underline font-semibold" disabled={syncing} onClick={() => void claimLegacy()}>
            Spara dem på mitt konto
          </button>
        )}
